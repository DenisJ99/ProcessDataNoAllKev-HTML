...
def extract_data(text):
    ...
    thread_kernel_counts = defaultdict(
        lambda: defaultdict(lambda: defaultdict(int))
    )  # New dictionary to store kernel call counts
    thread_running_time = defaultdict(
        lambda: defaultdict(lambda: {"total": 0, "msec": 0, "cpu_usage": 0})
    )  # New dictionary to store running time
    ...
    for line in text.split("\n"):
        ...
        if current_pid:
            ...
            for event in event_counts:
                if event in line and current_tid and event not in counted_events:
                    event_counts[event].setdefault(current_pid, {}).setdefault(
                        current_tid, 0
                    )
        ...
        if cpu_match:
            ...
            thread_running_match = re.search(
                r"THREAD\s+:THRUNNING\s+pid:(\d+)\s+tid:(\d+)", line
            )
        ...
        if time_match:
            timestamp = (
                int(time_match.group(1)) * 1_000_000
                + int(time_match.group(2)) * 1_000
            ...
            if thread_running_match:
                ...
                if thread_match:
                    ...
                    if tid in running_start_times[pid]:
                        ...
                        thread_running_time[pid][tid]["msec"] = (
                            thread_running_time[pid][tid]["total"] / 1_000
                        )
    ...
    with open(output_file, "w", encoding="utf-8") as file:
        ...
        file.write(
            ...
            function filterByProcess(fileNumber) {
                ...
                if (selectedProcess === 'all') {
                    threadTable.style.display = 'none';  // Hide thread table for all processes
            ...
            function showTable(tableId, fileNumber) {
                var allTables = ['threadTable', 'kernelTable', 'runningTimeTable'];
        ...
        for i, file_name in enumerate(file_names):
            ...
            file_display_name = os.path.basename(file_name).replace(".txt", "")
            file.write(f"<option value='file{i+1}'>{file_display_name}</option>")
        file.write("</select>")
        ...
        for i, (
            ...
            thread_kernel_counts,
            thread_running_time,
        ) in enumerate(extracted_data_list):
            file_number = i + 1
            file.write(
                f"<div id='file{file_number}' class='file-container' style='display:none;'><h2>{os.path.basename(file_names[i]).replace('.txt', '')}</h2>"
            )
            file.write(
                f"View: <select id='viewSelect{file_number}' onchange='showTable(this.value, {file_number})'>"
            )
            ...
            file.write("</select>")
            file.write(
                f"Select a process: <select id='processSelect{file_number}' onchange='filterByProcess({file_number})'>"
            )
            file.write("<option value='all'>All Processes</option>")
            for pid, pname in process_names.items():
                pname_display = pname.split("/")[
                    -1
                ].capitalize()  # Extract the name after the last "/" and capitalize it
                file.write(
                    f"<option value='{pid}'>{pname_display} (PID: {pid})</option>"
                )
            file.write("</select>")
            file.write(
                f" Select a thread name: <select id='threadNameSelect{file_number}' onchange='filterByThreadName({file_number})'>"
            )
            ...
            file.write(
                f'<div class="chart-container"><canvas id="line-chart{file_number}"></canvas><canvas id="bar-chart{file_number}"></canvas></div>'
            )
            ...
            for header in headers[1:]:
                file.write(
                    f"<th>{header} <button onclick=\"sortTable('{header}', 'asc', {file_number})\">&#9650;</button><button onclick=\"sortTable('{header}', 'desc', {file_number})\">&#9660;</button></th>"
                )
            file.write("</tr>")
            ...
            for pid, threads in data.items():
                for tid, name in threads.items():
                    ...
                    for event in headers[2:]:
                        file.write(
                            f"<td>{event_counts[event].get(pid, {}).get(tid, 0)}</td>"
                        )
                    file.write("</tr>")

            file.write(
                f"<tr class='totals-row'><td colspan='2'><strong>Totals</strong></td>"
            )
            for event in headers[2:]:
            ...
            # Adding the kernel call count table
            file.write(
                f"<table id='kernelTable{file_number}' style='display: none;'><tr>"
            )
            kernel_headers = ["Thread Name", "Thread ID"] + sorted(
                set(
                    k
                    for pid in thread_kernel_counts
                    for tid in thread_kernel_counts[pid]
                    for k in thread_kernel_counts[pid][tid]
                    if thread_kernel_counts[pid][tid][k] > 0
                )
            )
            ...
            for pid, threads in thread_kernel_counts.items():
                for tid in sorted(threads.keys(), key=lambda x: int(x)):
                    ...
                    for header in kernel_headers[2:]:
                        file.write(
                            f"<td data-header='{header}'>{threads[tid].get(header, 0)}</td>"
                        )
                    file.write("</tr>")
            file.write("</table>")

            # Adding the running time table
            file.write(
                f"<table id='runningTimeTable{file_number}' style='display: none;'><tr>"
            )
            running_time_headers = [
                "Thread Name",
                "Thread ID",
                "Running Time",
                "Running Time (MSEC)",
            ...
            for header in running_time_headers:
                if header in [
                    ...
                    file.write(
                        f"<th>{header} <button onclick=\"sortRunningTimeTable('{header}', 'asc', {file_number})\">&#9650;</button><button onclick=\"sortRunningTimeTable('{header}', 'desc', {file_number})\">&#9660;</button></th>"
                    )
                else:
                    file.write(f"<th>{header}</th>")
            file.write("</tr>")
...
def main():
    parser = argparse.ArgumentParser(description="Process kernel trace files.")
    parser.add_argument(
        "-w", "--wide_trace", required=True, help="Path to the wide trace KEV file."
    )
    parser.add_argument(
        "-f", "--filter_trace", required=True, help="Path to the filter trace KEV file."
    )
    parser.add_argument(
        "-t",
        "--traceprinter",
        required=True,
        help="Path to the traceprinter executable.",
    )
    parser.add_argument("output_file", help="Path to the output HTML file.")

    args = parser.parse_args()

    wide_trace_path = args.wide_trace
    filter_trace_path = args.filter_trace
    traceprinter_path = args.traceprinter
    output_file = args.output_file

    # Convert KEV files to TXT if necessary
    kev_files = [wide_trace_path, filter_trace_path]
    converted_files = []
    for kev_file in kev_files:
        txt_file = kev_file.replace(".kev", "_converted.txt")
        if convert_kev_to_txt(kev_file, txt_file, traceprinter_path):
            converted_files.append(txt_file)
        else:
            print(f"Skipping {kev_file} due to conversion error.")
    ...
    for input_file in converted_files:
        try:
            ...
            print(f"File not found: {input_file}. Skipping this file.")
            continue

    # Extract the data from the text
    extracted_data_list = []
    for text, input_file in zip(texts, converted_files):
        (
            data,
            process_names,
            event_counts,
        ...
        # Apply the conditions for hiding data based on file names
        if "wide" in input_file.lower():
            thread_running_time = defaultdict(
                lambda: defaultdict(lambda: {"total": 0, "msec": 0, "cpu_usage": 0})
            )  # Reset running time data
        if "filter" in input_file.lower():
            thread_kernel_counts = defaultdict(
                lambda: defaultdict(lambda: defaultdict(int))
            )  # Reset kernel call data
    ...
    write_to_html(extracted_data_list, output_file, converted_files)
